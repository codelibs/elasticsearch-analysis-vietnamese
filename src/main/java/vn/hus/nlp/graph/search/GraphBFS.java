...
public class GraphBFS {
     ...
     * @param g
     */
    public GraphBFS(final IGraph g) {
        init(g);
     ...
     * @param u
     */
    public GraphBFS(final IGraph g, final int u) {
        init(g);
    ...
    private void search(final Edge edge) {
        ...
        final Queue<Edge> queue = new ArrayBlockingQueue<>(n);
        queue.add(edge);
        order[edge.getV()] = count++;
        while (!queue.isEmpty()) {
            // get an edge from the queue
            final Edge e = queue.remove();
            final int u = e.getU(); // the start vertex of e
            final int v = e.getV(); // the end vertex of e
     ...
     * @param v
     */
    public void shortestPath(final int u, final int v) {
